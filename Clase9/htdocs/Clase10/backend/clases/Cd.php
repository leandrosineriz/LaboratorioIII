...
class CD{
    ...
    public static function TraerCds()
    {
        ...
        $response='<table class="table table-hover"><thead><tr><th scope="col">Album</th><th scope="col">Interprete</th><th scope="col">Año</th><th scope="col">Opciones</th></tr><tbody>';

        while($fila= $r->fetch())
        {      
            $response.='<tr><td>'.$fila[0].'</td><td>'.$fila["interprete"].'</td><td>'.$fila[2].'</td>';
            $response.='<td><button type="button" class="btn btn-primary btn-lg" onclick="Modificar()">Modificar</button>';
            $response.='<button type="button" class="btn btn-primary btn-lg" onclick="Eliminar()">Eliminar</button></td>';
            $response.="</tr>";
        }
